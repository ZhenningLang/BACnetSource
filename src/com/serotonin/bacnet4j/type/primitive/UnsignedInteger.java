...
public class UnsignedInteger extends Primitive {
    ...
    //
    /**
     * 从 queue 中读取(pop)一个无符号整数类型并存在本地
     * */
    public UnsignedInteger(ByteQueue queue) {
        int length = (int) readTag(queue); // 实际上 long 更合适
        if (length < 4) { // int 可以存下
            while (length > 0)
    ...
    protected long getLength() {
        if (bigValue == null) {
            int length;
            if (smallValue < 0x100) // 256
                length = 1;
            else if (smallValue < 0x10000) // 65536
                length = 2;
            else if (smallValue < 0x1000000) // 16777216
                length = 3;
            else
                length = 4;
