...
/**
 * 可编码的ip地址信息，包含 <br/>
 * Unsigned16 networkNumber - 网络号 <br/>
 * OctetString macAddress - mac 地址（或 ip + 端口号） <br/>
 * 写入和读取都是 "application" tags（见条款 20.2）
 * */
public class Address extends BaseType {
    ...
    /**
     * 将 networkNumber 和  MacAddress 写入队列 queue 中
     * 实际调用的是 Unsigned16 (-> UnsignedInteger -> Primitive) 的 write 函数写入 networkNumber
     * OctetString (-> Primitive)  的 write 函数写入 macAddress
     * 
     * Primitive 的 write 函数首先向 queue 中统一写入 tag
     * 再分别调用  UnsignedInteger 和  OctetString 的 writeImpl 函数写入具体的数据
     * */
    @Override
    ...
    /**
     * 将 networkNumber 和  MacAddress 从队列 queue 中读出
     * */
