...
/**
 * 用于进行网络层的封装，主要逻辑是收发函数，其中收函数实现了 Runnable 接口
 * */
public class IpNetwork extends Network implements Runnable {
    ...
    /**
     * 构造函数，其他构造函数都最终调用了这个构造函数，个参数介绍如下
     * @param broadcastIp - 广播ip地址，缺省值是 "255.255.255.255"
     * @param port - 本地端口号，缺省值是 47808
     * @param localBindAddress - 本地ip，如果有多个ip实际是绑定的ip，可以人为指定，缺省值是"0.0.0.0"
     * @param localNetworkNumber - 本地 BACnet 网络号，缺省值是 0，A value of 0 indicates the local network
     * */
    ...
    /**
     * 返回最大长度是1476 <br/>
     * 这是 ISO 8802-3（以太网）的网络层允许的最大报文长度（条款6表6-1）
     * */
    @Override
    ...
    /**
     * 封装 APDU 为 BVLL 报文：<br/>
     * 报文结构：[BVLL头部 - [NPCI - [APCI - [USER_DATA]]]]
     * BVLL: 条款 J.2
     * NPCI: 条款 6.2
     * */
    @Override
    public void sendAPDU(Address recipient, OctetString link, APDU apdu, boolean broadcast) throws BACnetException {
        ...
        // BACnet virtual link layer detail  -------------------- BVLL begin
        queue.push(0x81); // BACnet/IP
        queue.push(broadcast ? 0xb : 0xa); // Original-Unicast-NPDU, or Original-Broadcast-NPDU
        // ----------------------------------------------------- BVLL end
        
        // ----------------------------------------------------- NPCI begin
        ByteQueue postLength = new ByteQueue(); // postLength 中是 NPDU
    ...
    /**
     * Convenience methods
     * 将 InetAddress 转化为 com.serotonin.bacnet4j.type.constructed.Address
     * */
